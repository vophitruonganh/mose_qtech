<table class="table table-hover">
    <thead>
        <tr>
            ...
            ?>
            <th class="col-4 text-nowrap text-xs-center">Ngày đăng <a href="{{ url('admin/post?sort='.$sort.$url_sort)}}"><i class="font-icon material-icons md-18">swap_vert</i></a></th>
        ...
        @else
            ...
            @foreach( $posts as $post )
                ...
                <tr>
                    ...
                    <td class="col-2">@if($post_status=='trash' || $post->post_status == 'trash') <div class="title-text text-muted">{{ $title }}</div> @else <div class="title-link"><a href="{{ url('admin/post/edit/'.$post->post_id) }}">{{ $title }}</a></div>@endif</td>
                    <td class="col-3"><?php echo get_cat_post( $post->post_id) ?></td>
                    <td class="col-4 text-nowrap text-xs-center">{{ date('H:i d/m/Y',$post->post_date) }}</td>
                    <td class="col-5 text-nowrap text-xs-center">@if( $post->post_status == 'public' ) <span class="label label-success">Công khai</span> @elseif( $post->post_status == 'pending' ) <span class="label label-primary">Đang chờ duyệt</span> @elseif( $post->post_status == 'draft' ) <span class="label label-default">Nháp</span> @elseif( $post->post_status == 'trash' ) <span class="label label-danger">Xóa</span> @endif</td>
                </tr>
            @endforeach
        @endif
    </tbody>
