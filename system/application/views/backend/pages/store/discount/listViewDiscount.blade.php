<table class="table table-hover">
    ...
    <?php $i++; ?>
    <tr data-id="{{ $discount->discount_id }}" data-take="{{ $discount->discount_take }}" data-option="{{ $discount->discount_option }}">
        <th class="table-check"><input id="tbl-check-{{$i}}" type="checkbox" class="filled-tbl" name="check[]" id="check[]" value="{{ $discount->discount_id }}" /><label for="tbl-check-{{$i}}"></label></th>
        <td class="col-2">
            <div class="discount-type" data-type="{{ $discount->discount_type }}" data-promotion="{{ $discount->promotion_allow }}">@if( $discount->discount_type == 1 ) Mã khuyến mãi: @else Chương trình khuyến mãi: @endif</div>
            <div class="title-link discount-title"><a href="javascript:;">{{ $discount->discount_title }}</a></div>
        </td>
        <?php
        	$date_end = '-';
        ...
        ?>
        <td>
          <div>Bắt đầu: <span class="discount-date-start" data-text="{{ $discount->discount_date_start }}">{{ date('h:i d-m-Y',$discount->discount_date_start) }}</span></div>
          <div>Kết thúc: <span class="discount-date-end" data-text="{{ $discount->discount_date_end }}">{{ $date_end }}</span></div>
        </td>
        <?php
        ...
        ?>
        <td class="col-4 text-nowrap text-xs-center"><span class="discount-limit" data-limitstart="{{ $discount->discount_limit_start }}" data-limitend="{{ $discount->discount_limit_end }}">{{ $discountLimitTimeUsed }}</span></td>
          <?php
            $status = '';
          ...
          ?>
        <td class="col-5 text-nowrap text-xs-center">{!! $status !!}</td>
    @endforeach
    </tbody>
    @endif
</table>
