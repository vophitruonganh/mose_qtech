...
?>
    <div class="section-discount">
        ...
        <form id="discount-form" action="{{ url('admin/discount/create') }}" method="post" data-parsley-validate>
                    ...
                    <div class="box-typical b-t-p b-t-m">
                        <div class="form-group">
                            ...
                            <select id="discount-type" name="discount_type" class="form-control" data-bind="change: Discount.TypeChange, change: Discount.OfferOption">
                                <option value="1" @if( old('discount_type') == 1 ) selected="selected" @endif >Mã khuyến mãi (Coupon)</option>
                                <option value="2" @if( old('discount_type') == 2 ) selected="selected" @endif >Chương trình khuyến mãi</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label id="discount-title-lbl">Tên mã khuyến mãi</label>
                            <div class="input-group">
                                <input type="text" class="form-control" id="discount-title" name="discount_title" value="{{ old('discount_title') }}" required data-parsley-trigger="change focusout" data-parsley-required-message="Bạn không được để trống trường này" data-parsley-required="true" data-parsley-errors-messages-disabled />
                        ...
                        <div class="form-inline">
                                ...
                                <div class="input-group">
                                    ...
                                    <span class="input-group-btn">
                                        <select id="discount-option" name="discount_option" class="form-control" data-model="discount_option" data-bind="change: Discount.OptionChange, change: Discount.OfferOption">
                                            <option value="amount" @if( old('discount_option') == 'amount' ) selected="selected" @endif >VNĐ</option>
                                            <option value="percentage" @if( old('discount_option') == 'percentage' ) selected="selected" @endif >%</option>
                                        </select>
                                    </span>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" id="discount-take" name="discount_take" value="{{ old('discount_take') }}" data-bind="keyup: Discount.TakeSet" required data-parsley-trigger="change focusout" data-parsley-required-message="Bạn không được để trống trường này" data-parsley-required="true" data-parsley-errors-messages-disabled />
                            ...
                            <div class="form-group m-b-0">
                                <select id="discount-offer" name="discount_offer" class="form-control" data-bind="change: Discount.OfferOption">
                                    <option value="all" @if( old('discount_offer') == 'all' ) selected="selected" @endif >Tất cả đơn hàng</option>
                                    <option value="amount_order" @if( old('discount_offer') == 'amount_order' ) selected="selected" @endif >Trị giá đơn hàng</option>
                                    <option value="product_group" @if( old('discount_offer') == 'product_group' ) selected="selected" @endif >Nhóm sản phẩm</option>
                                    <option value="product" @if( old('discount_offer') == 'product' ) selected="selected" @endif >Sản phẩm</option>
                                    <option value="customer_group" @if( old('discount_offer') == 'customer_group' ) selected="selected" @endif >Nhóm khách hàng</option>
                                </select>
                            </div>
                        </div>
                        <div id="offer-content" class="m-t-1" data-bind="load: Discount.OfferOption, load: Discount.OfferItemSet" >
                            <div id="offer-amount-order" class="offer-content-group" style="display: none;">
                                <div class="form-group">
                                    <div class="input-group"><span class="input-group-addon">Từ</span><input type="text" class="form-control" id="money" name="money" value="{{ old('money') }}" data-bind="keyup: Format.Number" /><span class="input-group-addon">VNĐ</span></div>
                                </div>
                            </div>
                                ...
                                <div id="offer-option-1" class="offer-option-group" style="display: none;">
                                    <div class="form-group m-b-0">
                                        <select class="form-control" name="offer_option_1">
                                            <option value="per_order" @if( old('offer_option_1') == 'per_order' ) selected="selected" @endif >Một lần trên một đơn hàng</option>
                                            <option value="per_every_item" @if( old('offer_option_1') == 'per_every_item' ) selected="selected" @endif >Cho từng mặt hàng trong giỏ hàng</option>
                                        </select>
                                    </div>
                                </div>
                    ...
                    <div class="box-typical">
                        ...
                        <div class="proj-page-section proj-page-info">
                            <div class="form-group">
                                ...
                                <input type="text" name="start_date" id="start-date" class="form-control" data-plugin="datetimepicker" value="{{ old('start_date',date('H:i d/m/Y',time())) }}" placeholder="Ngày bắt đầu" />
                            </div>
                            <div class="form-group">
                                <label for="">Kết thúc khuyến mãi</label>
                                <input type="text" name="end_date" id="end-date" class="form-control" data-plugin="datetimepicker" value="{{ old('end_date') }}" placeholder="Ngày kết thúc" readonly="readonly" />
                            </div>
                            <div class="form-group m-b-0">
