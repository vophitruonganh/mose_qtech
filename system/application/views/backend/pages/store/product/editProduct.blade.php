...
?>
    <div class="section-product">
        ...
        <form id="product-form" action="{{ url('admin/product/edit/'.$post->product_id) }}" method="post">
                ...
                <div class="box-container-1">
                    <div class="box-typical b-t-p b-t-m">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control meta-title"  name="title" value="{{ old('title',$post->product_title) }}" />
                        </div>
                        <div class="form-group">
                            <div class="slug-group input-group">
                                <label class="sr-only">Đường dẫn</label>
                                <span class="input-group-addon">{{ main_site_url() }}/collections/</span>
                                <input type="text" name="slug" value="{{ old('slug',$post->product_slug) }}" class="form-control" readonly="readonly" />
                        ...
                        <div class="form-group">
                            <textarea class="form-control" name="content" data-plugin="tinymce">{{ old('content',$post->product_content) }}</textarea>
                        </div>
                        <div class="form-group">
                            <label>Mô tả</label>
                            <textarea rows="3" class="form-control meta-description" name="excerpt">{{ old('excerpt',$post->product_excerpt) }}</textarea>
                    ...
                    <div class="box-typical b-t-p b-t-m">
                        <div class="image-group">
                            ...
                            <div class="image-group-main">
                                <div class="image-group-info" @if(!$productImage)style="display: none;"@endif>
                                    <i class="font-icon material-icons">image</i>
                                    <p class="font-lg-size">Sử dụng nút Chọn hình để thêm hình vào sản phẩm.</p>
                                </div>
                                <div class="image-group-list" @if(!$productImage)style="display: none;"@endif>
                                    <ul class="clearfix" data-plugin="sortable">
                                        @foreach( $productImage as $image )
                                        <li class="box-product-images ui-sortable-handle"><input type="hidden" name="product_image[]" value="{{$image}}" /><div class="thumbnail-preview"><div class="thumbnail"><div class="centered"><img src="{{ get_image($image,'thumb') }}" /></div></div></div></li>
                                        @endforeach
                                    </ul>
                                </div>
                            </div>
                        ...
                        <div class="box-typical-body b-t-p">
                            <div class="box-typical-heading heading-group">
                                ...
                                <div class="heading-action">
                                    ...
                                    <button type="button" class="btn btn-link" data-bind="click: Variant.LoadOptionModal">Chỉnh sửa</button>
                                    <button type="button" class="btn btn-link" data-bind="click: Variant.LoadAddModal">Thêm mới</button>
                                </div>
                            </div>
                            <div class="variant-table" data-bind="load: Variant.LoadImageModal, load: Variant.LoadEditModal, load: Variant.Delete" data-product-id="{{ $post->product_id }}">
                                @include('backend.pages.store.product.listVariant')
                            </div>
                        </div>
                    </div>
                    <?php /*
                    ...
                    <div class="box-typical">
                        <div class="proj-page-section proj-page-action clearfix">
                            <div class="pull-xs-right">
                                <a class="btn btn-link text-danger" href="{{ url('admin/product/delete/'.$post->product_id) }}">Xóa sản phẩm</a>
                            ...
                            <div class="misc-actions">
                                <div class="misc-group">
                                    <div data-toggle="collapse" data-target="#misc-product-status" aria-expanded="false" aria-controls="misc-status-select">
                                        ...
                                        <label class="misc-label-text m-b-0" data-bind="text: product_status">@if( $post->product_status == 'public' ) Công khai @elseif( $post->product_status == 'pending' ) Đang chờ duyệt @elseif( $post->product_status == 'draft' ) Nháp @elseif( $post->product_status == 'trash' ) Xóa @endif</label>
                                        <i class="font-icon material-icons">mode_edit</i>
                                        <input type="hidden" name="hidden_product_status" id="hidden_product_status" value="{{old('product_status','public')}}" data-bind="value: product_status" />
                                    </div>
                                    <div id="misc-product-status" class="form-inline collapse extend">
                                        <div class="form-group">
                                            <select name="product_status" id="product_status" class="form-control misc-select-text" data-model="product_status">
                                                <option @if($post->product_status=='public'){{'selected="selected"'}} @endif value="public">Công khai</option>
                                                <option @if($post->product_status=='pending'){{'selected="selected"'}}@endif value="pending">Đang chờ duyệt</option>
                                                <option @if($post->product_status=='draft'){{'selected="selected"'}}@endif value="draft">Nháp</option>
                                            </select>
                                        </div>
                                ...
                                <div class="misc-group">
                                    <div class="inner" data-toggle="collapse" data-target="#misc-product-date" aria-expanded="false" aria-controls="misc-product-date">
                                        ...
                                        <label class="misc-label-text m-b-0" data-bind="text: product_date">{{ date('H:i d/m/Y',$post->product_date)}}</label>
                                        <i class="font-icon material-icons">mode_edit</i>
                                    </div>
                                    <div id="misc-product-date" class="form-inline collapse extend">
                                        <div class="form-group">
                                            <input type="text" name="product_date" class="form-control" data-plugin="datetimepicker" value="{{ date('H:i d/m/Y',$post->product_date)}}" placeholder="Ngày tạo" data-model="product_date" />
                                        </div>
                                ...
                                <div class="misc-group">
                                    <div class="inner" data-toggle="collapse" data-target="#misc-product-comment" aria-expanded="false" aria-controls="misc-product-comment">
                                        ...
                                        <label class="misc-label-text m-b-0" data-bind="text: product_comment">@if( $post->comment_status == 'yes' ) Mở @elseif( $post->comment_status == 'no' ) Đóng @endif</label>
                                        <i class="font-icon material-icons">mode_edit</i>
                                        <input type="hidden" name="hidden_product_comment" id="hidden_product_comment" value="{{$post->comment_status}}" data-bind="value: post_comment" />
                                    </div>
                        ...
                        <div class="proj-page-section">
                            ...
                            <div class="dropdown-select" data-slug="product-group" data-name="product_group" data-type="multiple">
                                <div class="dropdown">
                                    <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdown-product-group" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="text pull-left" data-title="Chọn nhóm sản phẩm">Chọn nhóm sản phẩm <span class="count-selected" @if(!$product_group)style="display: none;"@endif><?php if(!$product_group){ echo '(0)'; }else{ echo '('.count($product_group).')';}?></span></span></button>
                                    <div class="dropdown-menu" aria-labelledby="dropdown-product-group">
                                        ...
                                        <ul>
                                            @foreach($product_group_lists as $product_group_list)
                                            <li @if(in_array($product_group_list['taxonomy_id'],$product_group))class="selected" @endif><a class="dropdown-item" href="javascript:;" data-id="{{$product_group_list['taxonomy_id']}}"><span>{{$product_group_list['taxonomy_name']}}</span><i class="font-icon material-icons md-18">check</i></a></li>
                                            @endforeach
                                        </ul>
                                    </div>
                                    @if($product_group)
                                        @foreach($product_group as $item)
                                            <input class="product-group product-group-{{$item}}" type="hidden" name="product_group[]" value="{{$item}}" />
                                        @endforeach
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="dropdown-select" data-slug="product-vendor" data-name="product_vendor" data-type="single">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdown-product-vendor">
                                        ...
                                        <ul>
                                            @foreach($product_vendor_lists as $product_vendor_list)
                                            <?php if(in_array($product_vendor_list['taxonomy_id'],$product_vendor)){ $title = $product_vendor_list['taxonomy_name'];}else { $title ='';} ?>
                                            <li @if(in_array($product_vendor_list['taxonomy_id'],$product_vendor))class="selected" @endif><a class="dropdown-item" href="javascript:;" data-id="{{$product_vendor_list['taxonomy_id']}}"><span>{{$product_vendor_list['taxonomy_name']}}</span><i class="font-icon material-icons md-18">check</i></a></li>
                                            @endforeach
                                        </ul>
                                    </div>
                                    @if($product_vendor)
                                        @foreach($product_vendor as $item)
                                            <input class="product-vendor product-vendor-{{$item}}" type="hidden" name="product_vendor[]" value="{{$item}}" />
                                        @endforeach
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="proj-page-section">
                            ...
                            <div class="form-group m-b-0">
                                <input type="text" name="newtags" class="form-control" data-plugin="tagsinput" value="{{old('tag_name',$tag_name)}}" placeholder="Nhập nhãn bài viết" />
                            </div>
                        </div>
                        <?php
                        /*
    ...
    <div id="modal-option-edit" class="modal fade">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ url('admin/product/variant/option/'.$post->product_id) }}" method="post" data-bind="form: disable">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" data-bind="click: Variant.OptionModal">Lưu thay đổi</button>
                </div>
                </form>
            </div>
        </div>
    </div>
    <div id="modal-variant-add" class="modal fade" data-bind="load: Variant.SubmitModal" data-type="create">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ url('admin/product/variant/create/'.$post->product_id) }}" method="post" data-bind="form: disable">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Hủy</button>
                        <button type="button" class="btn btn-primary btn-submit">Thêm mới</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div id="modal-variant-sort" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ url('admin/product/variant/sort/'.$post->product_id) }}" method="post" data-bind="form: disable">
    ...
    <div id="modal-variant-image" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ url('admin/product/variant/image/'.$post->product_id) }}" method="post" data-bind="form: disable">
                    <div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title">Cập nhật hình ảnh phiên bản</h4></div>
                    <div class="modal-body">
                        <div class="variant-image-data"></div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Hủy</button>
                        <button type="button" class="btn btn-primary" data-bind="click: Variant.ImageModal">Lưu thay đổi</button>
                    </div>
                </form>
            </div>
        </div>
