...
<p>Cảm ơn Anh/chị đã đặt hàng tại <strong>{{url('/')}}!</strong></p>
...
<table>
 <tbody>
  ...
  <tr>
   <td>Email</td>
   <td><a href="mail to:trandotheanh@gmail.com" target="_blank">{{$email}}</a></td>
  </tr>
  <tr>
   <td>Địa chỉ</td>
...
<table>
 <tbody>
  <tr>
   <td><strong>Mã đơn hàng:</strong> {{get_template_order_code($order->order_code)}}</td>
   <td><strong>Ngày tạo:</strong> {{date('d/m/Y',$order->date_create)}}</td>
  </tr>
 </tbody>
</table>
<br>
...
<table>
 <tbody>
  ...
  <tr>
   <td>{{ $product->title}} - {{$product->variant_name}}</td>
   <td style="text-align:center;width:100px">{{$product->quantity_buy}}</td>
   <td style="text-align:right;width:150px">{{ number_format($product->quantity_buy* $product->price, 0, ',', '.') }}&#8363;</td>
  </tr>
  @endforeach
  <tr>
   <td>&nbsp;</td>
   ...
   <table style="width:100%">
    <tbody>
     <tr>
      ...
      <td style="width:150px">{{number_format($order->amount_discount, 0, ',', '.') }}&#8363;</td>
     </tr>
     <tr>
      <td><strong>Tổng giá trị sản phẩm:</strong></td>
      <td style="width:150px">{{number_format($order->amount_order, 0, ',', '.') }} &#8363;</td>
     </tr>
     <tr>
     ...
     <tr>
      ...
      <td style="width:150px">{{number_format($order->amount_payment, 0, ',', '.') }} &#8363;</td>
     </tr>
    </tbody>
   </table>
   </td>
...
Ban quản trị <a href="{{url('/')}}" target="_blank" data-saferedirecturl="https://www.google.com/url?hl=vi&amp;q=http://office365-theme.bizwebvietnam.net&amp;source=gmail&amp;ust=1478772361307000&amp;usg=AFQjCNEtsPtQgzTjGXqDkY8otp7Ye3CLeA">{{url('/')}}</a></div>
