...
<div class="main-container col2-left-layout">
   ...
   <div class="category-products">
      <div class="toolbar">
         <div class="sorter">
            <div class="view-mode"> 
               <a href="{{url('collections/'.$slug.'?view=grid')}}" title="Grid" class="button-active"><i class="fa fa-th"></i></a> 
      ...
      <ol id="products-list" class="products-list">
        @if(count($products)>0)
            @foreach($products as $product)
             <li class="item even">
                <div class="product-image"> <a href="{{url('collections/'.$product['product_slug'])}}" title="Xe đạp gấp Fornix FB2007-PRA14"> <img class="small-image" src="{{get_image_url($product['product_image_id'])}}" width="230"> </a> </div>
                <div class="product-shop">
                   <h3 class="product-name"><a title="Xe đạp gấp Fornix FB2007-PRA14" href="{{url('collections/'.$product['product_slug'])}}">{{$product['product_title']}}</a></h3>
                   <div class="desc std">
                      <p>{{$product['product_excerpt']}}<a class="link-learn" title="" href="{{url('collections/'.$product['product_slug'])}}">Xem thêm</a> </p>
                   </div>
                   <div class="price-box">
                      @if($product['price_old']>0)
                        <p class="old-price"> <span class="price-label"></span> <span id="old-price-212" class="price">{{number_format($product['price_old'],0,'.','.')}}₫</span> </p>
                      @endif
                      <p class="special-price"> <span class="price-label"></span> <span id="product-price-212" class="price">{{number_format($product['price_new'],0,'.','.')}}₫</span> </p>
                   </div>
