...
<div id="maincontainer">
    <div class="container">
        <section id="product">
            ...
            <div class="row">
                ...
                <div class="col-lg-4">
                    <div id="surround">
                        <!--
                        <img class=" product-image-feature" src="//hstatic.net/025/1000032025/1/2015/8-7/ao-so-mi-1_master.png" alt="{{ $title }}">
                        -->
                        <img style="" class="product-image-feature" src="{{ $featureImage }}" alt="{{ $title }}">
                        <div id="sliderproduct" class="">
                            <ul class="slides" >
                            @if(count($list_post_image)>0)
                                @foreach($list_post_image as $img)
                                <li class="product-thumb @if($img ==$product['product_image_id'] ) active @endif">
                                    <div class="img-thumb-wp">
                                        <a href="#" data-image="{{ get_image_url($img) }}" data-zoom-image="{{ get_image_url($img) }}">
                                            <img style="width: 80px; height: 80px" alt="{{ $title }}" src="{{ get_image_url($img) }}" />
                                        </a>
                                    </div>
                                </li>
                                @endforeach
                ...
                <div class="col-lg-8">
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="row">
                                <div class="col-lg-6">
                                    <h1 class="productname"><span class="bgnone">{{ $title }}</span></h1>
                                    <div class="variant">
                                    ...
                                    <div class="productprice">
                                        <div class="productpageprice">
                                            <span class="spiral"></span>{{ $priceNew }}₫
                                        </div>
                                        @if($priceOld>0)
                                        <div class="productpageoldprice">
                                            <span>Giá cũ :</span> {{ $priceOld }}₫
                                        </div>
                                        @endif
                                    </div>
        ...
        <?php $product_relateds = get_product_related('product_category', $slug, 4 ) ?>
        @if($product_relateds)
         <div id="like">
            <h1 class="heading1"><span class="maintext">Sản phẩm khác</span></h1>
            <ul class="thumbnails">
               @foreach( $product_relateds as $product_rel)
               <li class="col-lg-3 col-sm-3">
                  <a class="prdocutname" href="{{url('collections/'.$product_rel['product_slug'])}}" title="{{$product_rel['product_title']}}">{{$product_rel['product_title']}}</a>
                  <div class="thumbnail">
                     <a href="{{url('collections/'.$product_rel['product_slug'])}}" title="{{$product_rel['product_title']}}"><img alt="{{$product_rel['product_title']}}" src="{{get_image_url($product_rel['product_image_id'])}}" alt="" /></a>
                     <div class="pricetag">
                        <form id="formOrder{{ $product_rel['product_id'] }}" action="{{ url('cart/order/'.$product_rel['product_slug']) }}" method="post">
                              <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                              <input type="hidden" name="quantity" value="1" />
                          </form>
                        <span class="spiral"></span><a onclick="order({{ $product_rel['product_id'] }})" class="productcart">Mua ngay</a>
                        <div class="price">
                           <div class="pricenew">{{number_format($product_rel['price_new'],0,'.','.')}}&#8363;</div>
                        </div>
                     </div>
                  </div>
               </li>
                   ...
                   $.ajax({
                        ...
                        success: function(data){
                            if(data ==0){
                                ...
                                $('.productpageprice').html('');
                                $('.productpageoldprice').html('<span class="spiral"></span>'+data.price_old);
                            }else{
                                $('.product-image-feature').attr('src',data.image);
                                $('.productpageprice').html('<span class="spiral"></span>'+data.price_new);
                                $('.productpageoldprice').html('<span class="spiral">Giá cũ: </span>'+data.price_old);
