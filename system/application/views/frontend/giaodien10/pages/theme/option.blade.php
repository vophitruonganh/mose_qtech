...
@section('titlePage','Thiết lập giao diện')
@section('content')
<?php 
    $heading = array(
...
?>
	<div class="section-template">
							...
							<div class="box-typical b-t-p">
								...
								<div class="form-group">
									...
									<div class="input-group">
										<input type="text" name="favicon[image]" class="form-control" value="{{ isset($optionValue['favicon']['image']) ? $optionValue['favicon']['image'] : '' }}" />
								...
								<div class="form-group">
									<label>Logo</label>
									<div class="input-group">
										<input type="text" name="logo_main[image]" class="form-control" value="{{ isset($optionValue['logo_main']['image']) ? $optionValue['logo_main']['image'] : '' }}" />
										<span class="input-group-btn">
											<span class="fileUpload btn btn-primary">
	    										<span>Upload</span>
	    										<input type="file" name="file" class="upload" />
		...
		<?php /* </form> */ ?>
	<!-- </div> -->
<?php /* <form action="{{ url('admin/template/option') }}" method="post"> */ ?>
<?php /* @include('backend.includes.token') */ ?>
	<div class="theme">
	...
	*/
	...
	?>
	<div id="mini_slider" data-plugin="upload">
		<div><strong>Mini Slider:</strong><a class="addRow" href="#">Add</a></div>
		<?php $i = 0; ?>
		<div class="block">
		@foreach( $miniSlider as $row )
			<div class="row form-group" data-id="{{ $i }}">
				<div>Image: <input type="text" class="form-control" name="mini_slider[{{ $i }}][image]" value="{{ $row['image'] }}" /></div>
				<span class="input-group-btn">
					<span class="fileUpload btn btn-primary">
						<span>Upload</span>
						<input type="file" name="file" class="upload" />
				...
				<div>Url: <input type="text" name="mini_slider[{{ $i }}][url]" value="{{ $row['url'] }}" /></div>
				@if( $i > 0 ) <a class="remove" href="javascript:;">Remove</a> @endif
			</div>
		<br/>
		<?php $i++; ?>
	...
	<!-- <div id="url_website">
		...
		<div>Url: <input type="text" name="url_website[url]" value="{{ isset($optionValue['url_website']['url']) ? $optionValue['url_website']['url'] : '' }}" /></div>
	</div> -->
	<!-- End Url Website -->
	...
	<div id="contact_info">
		...
		<div>Phone 1: <input type="text" name="phone1[text]" value="{{ isset($optionValue['phone1']['text']) ? $optionValue['phone1']['text'] : '' }}" /></div>
		<div>Phone 2: <input type="text" name="phone2[text]" value="{{ isset($optionValue['phone2']['text']) ? $optionValue['phone2']['text'] : '' }}" /></div>
	</div>
	<!-- End contact -->
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#mini_slider .addRow').click(function(){
			...
			var row = '';
			row += '<div class="row form-group" data-id="'+numberMaxElement+'">';
			row += '<div>Image: <input type="text" class="form-control" name="mini_slider['+numberMaxElement+'][image]" value="" /></div>';
			row += '<span class="input-group-btn">';
			row += '<span class="fileUpload btn btn-primary">';
			row += '<span>Upload</span>';
			row += '<input type="file" name="file" class="upload" />';
