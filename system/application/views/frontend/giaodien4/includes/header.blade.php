	...
	<div class="container">
		...
		<div class="topbar-cart">
			<div class="shopping_cart">
				<a href="{{ url('cart') }}" title="giỏ hàng" rel="nofollow">
					<i class="fa fa-shopping-cart"></i>
					<span class="top-cart">Giỏ hàng :</span>
					<span class="ajax_cart_no_product ajax_cart_quantity">{{ $total_quantity }}</span>
				</a>
		...
		<div class="topbar-info">
			<div class="taikhoan">
				<?php
					...
					{
				?>
					<a class="link" href="{{ url('customer') }}">Chỉnh sửa</a>
					<a class="link" href="{{ url('customer/logout') }}">Thoát</a>
				<?php
					}
					else
					{
				?>
				<a class="link" href="{{ url('customer/register') }}" title="Đăng ký">Đăng ký</a>
				<span class="sep">|</span>
				<a class="link" href="{{ url('customer/login') }}" title="Đăng nhập">Đăng nhập</a>
				<?php
					}
				?>
			</div>
			<div class="mobile-menu-icon-wrapper">
				<ul class="mobile-menu-icon">
						...
						<div class="btn-group">
							...
							<div class="dropdown-menu" role="menu">
								<div class="search-bar">
									<div class="">
										<form class="col-md-12" action="{{url('collections')}}" method="get">
					...
					<li class="user"><a href="{{ url('customer/login') }}" title="Đăng nhập" class="fa fa-user"></a></li>
				</ul>
			</div>
		</div>
	</div>
	...
	<div class="container">
		<div class="row">
			<div class="header-logo col-md-3 col-xs-12">
				<h1><a href="{{ url('/') }}"><img src="{{ $logo_main }}" class="img-responsive"/></a></h1>
			</div>
			...
			<div class="header-search col-md-3">
				<form id="J_searchForm" class="search-form clearfix" action="{{ url('collections') }}" method="get">
					<label for="search" class="hide"></label>
					<?php $search = isset($_GET['search']) ? $_GET['search'] : ''; ?>
					<input class="search-text"  name="search" type="search" id="search" placeholder="Tìm kiếm..." value="{{ $search }}" />
