          ...
          @if( count($errors) > 0 )
           ...
           @endif
            <form accept-charset="UTF-8" action="{{url('user/register')}}" id="create_customer" method="post">
               ...
               <label for="LastName" class="hidden-label">Giới tính</label>
               <select name="gender">
               ...
               <input type="password" name="password_confirmation" id="CreatePassword" class="input-full" placeholder="Mật khẩu">
               <p>
                  <input type="submit" value="Tạo mới" class="btn btn--full">
               </p>
               <a href="{{url('collections')}}">Quay lại cửa hàng</a>
            </form>
         </div>
      </div>
   </div>
