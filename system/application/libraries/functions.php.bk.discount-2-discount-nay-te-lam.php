...
if( !function_exists('promotionOrder') )
{
	function promotionOrder($variants,$discountCode=null,$idCustomer=null)
	{
		...
		//$discountCode = 'WC1KYZRGQARK';
		$discountCode = '8LWRGT0UOSHF';
		$totalMoneyInOrder = 0;
		$resultVariant = [];

		// Variable Discount Group Product
		$arrayGroupProduct = [];
		$discountGroupProductId = 0;
		$arrayGroupProductDiscount = []; // [ 'variant_id_1' => 'group_product_id_1', 'variant_id_2' => 'group_product_id_2', ... ]
		// End
			...
			if( $product )
			{
				...
				foreach( $discounts as $discount )
				{
					if( $discount->discount_option == 'amount' && $discount->discount_take > $discountPriceAmount )
					{
						...
						$discountGroupAmount = $discount->relationship_id;
					}
					...
					if( $discount->discount_option == 'percentage' && $discount->discount_take > $discountPricePercentage )
					{
						...
						$discountGroupPercentage = $discount->relationship_id;
					}
				}
				//- End
				...
				$discountGroupProductId = $discountGroupPercentage;
				if( $discountPrice < $discountPriceAmount )
				{
					$discountPrice = $discountPriceAmount;
					$discountTitle = $discountTitleAmount;
					$discountGroupProductId = $discountGroupAmount;
				}

				if( $discountGroupProductId != 0 ) $arrayGroupProductDiscount[$variant['variant_id']] = $discountGroupProductId;
				...
				if( $discountPrice < $_discountPrice )
				{
					...
					$discountTitle = $discountTitlePercentage;
					unset($arrayGroupProductDiscount[$variant['variant_id']]);
				}
				if( $discountPrice < $discountPriceAmount )
				{
					$discountPrice = $discountPriceAmount;
		...
		print_r($arrayGroupProductDiscount);
		// ---------------------------------------
		// Discount for order
		// ---------------------------------------
