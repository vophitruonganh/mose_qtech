...
//            'keysSelectCategoryProduct' => $keysSelectCategoryProduct,
    ...
    public function update($slugMenuName,Request $request)
    {
        ...
        $navigation_load = decode_serialize($optionObject->getOptionValue_option('navigation_load')); //mảng null
        /*-- Get Menu Data From Template --*/
        $active_template = $optionObject->getOptionValue_option('active_template'); //giao dien 11
        require_once('system/application/views/frontend/'.$active_template.'/function.php');
        if( function_exists('resgister_navigation') )
        {
            $registerNavigation = resgister_navigation(); //top menu
        }
        else
        {
            return redirect('admin');
        ...
        // Delete
        unset($navigation_data[$slugMenuName]); // xóa navigation
        if( !is_array($navigation_load) )
        {
            $navigation_load = [];
        }
