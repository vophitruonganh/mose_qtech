<table class="table table-hover">
    ...
    <tr>
        ...
        <?php if($order['order_status']==3): ?>
            <td class="col-2"><div class="title-link"><a href="<?php echo e(url('admin/order/draft/'.$order['order_code'])); ?>"><?php echo e($order_code); ?></a></div></td>
        <?php else: ?>
            <td class="col-2"><div class="title-link"><a href="<?php echo e(url('admin/order/detail/'.$order['order_id'])); ?>"><?php echo e($order_code); ?></a></div></td>
        <?php endif; ?>
        <td class="col-3"><?php echo e(date('d-m-Y H:i',$order["order_date"])); ?></td>
        <td class="col-4"><a href="<?php echo e(url('admin/order?customer_id='.$order['customer_id'])); ?>" ><?php echo e($order["customer_fullname"]); ?></a></td>
        <td class="col-5 text-nowrap"><?php echo e(number_format($order["amount_order"],0,',',',')); ?> ₫</td>
        <td class="col-6 text-nowrap text-xs-center"><?php if( $order["order_status"]==0 ): ?> <span class="label label-success">Đã thanh toán</span> <?php elseif( $order["order_status"]==1 ): ?> <span class="label label-primary">Chưa thanh toán</span> <?php elseif( $order["order_status"]==3 ): ?> <span class="label label-default">Nháp</span> <?php elseif( $order["order_status"]==4 ): ?> <span class="label label-default">Hoàn trả một phần</span> <?php elseif( $order["order_status"]==5 ): ?> <span class="label label-default">Hoàn trả toàn bộ</span> <?php endif; ?></td>
    </tr>
