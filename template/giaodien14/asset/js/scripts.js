function scrollDirection(){
	var position = 100;//zjs(window).scrollTop();
	jQuery('.navi-right a').each(function(index, element){
		// element == this
		//var zEl = zjs(el);
	...
	jQuery(window).on('scroll', function () {
		...
		if (scroll > position) {
			jQuery('.navi-right').removeClass('up');
			jQuery('.navi-right').addClass('down');
		}else{
			jQuery('.navi-right').removeClass('down');
			jQuery('.navi-right').addClass('up');
		}
	});
}
