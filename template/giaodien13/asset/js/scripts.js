...
jQuery(document).ready(function($){
	$(".navbar-big .menu-item a").each(function(e){
			...
			if ( destination != '/' && window.location.pathname == '/' ) {
				...
				if ( $(destination).offset() != undefined ) {
					if ( $('.navbar').hasClass('navbar-mini')) {
						var scrollTo = $(destination).offset().top - 0;
					} else {
						var scrollTo = $(destination).offset().top - 0;
					}
	...
	$('.event-menu-main li').click(function(){
		$('.event-menu-main li').removeClass('current');
		index = $(this).index();
		$.each($('.event-menu-main'),function(i,v){
			$(this).find('li').eq(index).addClass('current');
		});
	});
});
/*-- End Link Scroll Menu -- */
