...
jQuery(document).ready(function($){
  ...
  $(window).scroll(function() {
    ...
    if (menutop_pos - windowpos > header_offset_top) {
      $('header.header').css('top',menutop_pos - windowpos);
    }else if($('header.header').css('top') != header_offset_top){
      $('header.header').css('top',header_offset_top);
    }
    if(windowpos > container_offset_top - $('header.header').outerHeight() ){
      $('header.header').addClass('menu-bg');
    } else {
      $('header.header').removeClass('menu-bg');
    }
  });
  ...
  $('#partners .slides').owlCarousel({
    items: 6,
    itemsCustom: [[0, 2], [480, 3], [768, 4], [860, 5],[960, 6]],
    pagination: false,
    navigation : true,
    slideSpeed : 600,
    paginationSpeed : 400,
