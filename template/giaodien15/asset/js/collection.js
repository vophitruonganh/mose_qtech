  ...
  var collection = { 
    ...
    fillterTagEvents: function() {
      ...
      $( "body" ).delegate( '.filter-tag a:not(".clear"), .filter-tag label', 'click',function(e) {           
        ...
        if (Haravan.queryParams.constraint) {
            currentTags = Haravan.queryParams.constraint.split('+');     
        }  
        ...
        if (currentTags.length) {
            Haravan.queryParams.constraint = currentTags.join('+');
        } else {
    ...
    filterCreateUrl: function(baseLink) {
      var newQuery = $.param(Haravan.queryParams).replace(/%2B/g, '+');
      if (baseLink) {
